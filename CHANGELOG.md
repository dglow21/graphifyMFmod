...
- Feat: mainframe code support — COBOL (`.cob .cbl .ccp .cobol .cob85 .scbl`), COBOL copybooks (`.cpy .copy .cbk`), JCL (`.jcl .proc`), PL/I (`.pli .pl1`), REXX (`.rexx .rex`), HLASM assembler (`.asm .mlc .hlasm .assemble`), IMS DBD/PSB (`.dbd .psb`), CICS BMS maps (`.bms .mapset`), and DB2/SQL DDL (`.ddl`)
- Feat: COBOL extraction emits programs, sections, paragraphs, `CALL`/`PERFORM`/`GO TO` call edges (attributed to the enclosing paragraph, with a string-literal guard), `CANCEL` references, `COPY` dependency edges, embedded `EXEC SQL` table/`INCLUDE` references and `EXEC CICS LINK`/`XCTL`/`SEND/RECEIVE MAP`/`MAPSET`/`FILE` references — handling both fixed- and free-format source, COBOL continuation lines, `REPLACE`/compiler directives, IDENTIFICATION-DIVISION comment entries, and nested/contained programs
- Feat: COBOL uses a robust regex-based structural extractor as its base layer; when the optional `antlr4-python3-runtime` is installed the file is *also* parsed with the vendored [ProLeap](https://github.com/uwol/proleap-cobol-parser) COBOL 85 ANTLR4 grammar (`.g4` + ANTLR-generated lexer/parser/listener under `graphify/_antlr/cobol85/`, SLL prediction, size-guarded) and its precise paragraph-scoped call edges are merged in (ANTLR-only "definitions" are discarded so a partial parse can't inject spurious nodes). Install with `pip install graphifyy[mainframe]`
...
- Feat: JCL extraction emits jobs, in-stream and cataloged procedures (`PROC`/`PEND`, or a bare proc member keyed on the file name), steps (`EXEC PGM=`/`PROC=`/`EXEC procname`), DD dataset reads/writes (including DSN on continuation lines), and `INCLUDE` members
- Feat: HLASM extraction emits control sections (`CSECT`/`RSECT`/`DSECT`/`START`/`COM`), macro definitions, `ENTRY`/`EXTRN`/`WXTRN` linkage, `COPY` includes, `=V(...)`/`=A(...)` address constants, and `CALL`/`LINK EP=`/`XCTL`/`ATTACH`/`LOAD` program invocations scoped to the enclosing section. IMS DBD/PSB extraction emits databases, segments and their parent hierarchy, PCBs, target databases and sensitive segments. BMS extraction emits mapsets and maps (map node ids match COBOL `EXEC CICS SEND/RECEIVE MAP`). PL/I emits procedures/entries, `%INCLUDE`, `FETCH` and `CALL`; REXX emits internal routines plus `CALL` and internal-function-call edges
- Feat: DB2/SQL `.ddl` is parsed with the tree-sitter SQL extractor when `tree-sitter-sql` is installed, otherwise a dependency-free DDL regex extractor (`CREATE TABLE`/`VIEW`/`INDEX`/`TABLESPACE`/`PROCEDURE`/`FUNCTION`/`TRIGGER`/..., `FOREIGN KEY ... REFERENCES`)
- Feat: COBOL `PROGRAM-ID`, JCL `PGM=`/`PROC=` targets, `CALL 'literal'` targets, IMS DBD/segment names, BMS map names, and copybook/`%INCLUDE` members share stable node ids, so JCL → program → sub-program chains, `COPY`/`%INCLUDE` dependencies, PSB → DBD → segment, and `EXEC CICS SEND MAP` → BMS link up across files
- Feat: COBOL embedded SQL ↔ DB2 DDL — SQL table/view nodes are now keyed on the unqualified table name (file-independent), so a COBOL `EXEC SQL ... FROM CUSTOMER` links to `CREATE TABLE APP.CUSTOMER` even when one is schema-qualified and the other isn't, and foreign keys resolve across DDL files. (`extract_sql`, used for `.sql` and `.ddl`, also now picks up table-level `FOREIGN KEY ... REFERENCES` constraints, not just column-level ones)
- Feat: COBOL ↔ IMS — a JCL IMS batch step (`EXEC PGM=DFSRRC00,PARM=(DLI|DBB,program,psb)`) emits `step → program executes` and `program → PSB uses`; a PSB is keyed on its member name (= the name JCL references) and `contains` its PCBs, which `use` their target DBDs, which `contain` their segments — so the full `JCL → program → PSB → PCB → DBD → segment` chain resolves across files
